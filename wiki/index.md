# Wiki Index

Master catalog of every page, grouped by entity type. Regenerated as pages are added — `/ingest` writes here.

Your wiki is currently empty. Drop source documents into `raw/<category>/` and run `/ingest` to start populating it.

---

## Sources

*No source pages yet.*

## Products

*No product pages yet.*

## Features

*No feature pages yet.*

## Personas

*No persona pages yet.*

## Customers

*No customer pages yet.*

## Competitors

*No competitor pages yet.*

## Concepts

*No concept pages yet.*

## Initiatives

*No initiative pages yet.*

## Decisions

*No decision pages yet.*

## Metrics

*No metric pages yet.*

## People / Teams

*No people pages yet.*

## Analyses

*No analyses yet.*

## Style Rules

*No style rules yet.*
